...
const Taskbar = () => {
  ...
  const filteredTasks = filter
    ? tasks.filter(
        (task) => task.difficulty === filter || task.frequency === filter
      )
    : tasks;
      ...
      <View className="p-4">
        ...
        <View className="flex-row justify-between items-center mb-4 relative">
          ...
          <View className="flex-row justify-between items-center bg-gray-100 p-2 rounded-lg">
            {["All", "One-Time", "Daily", "Weekly", "Monthly", "Annually"].map(
              (option) => (
                <TouchableOpacity
                  key={option}
                ...
                >
                  <Text
                    className={`text-sm font-semibold ${
                      filter === option ? "text-white" : "text-black"
                    }`}
                  >
                    {option}
                  </Text>
        ...
        >
          <View className="p-6 bg-white rounded-xl mx-6 mt-20">
            ...
            <View className="flex-row justify-center mt-3 space-x-4">
              {["Easy", "Intermediate", "Hard"].map((option) => (
                ...
                >
                  <Text
                    className={
                      difficulty === option ? "text-white" : "text-black"
                    }
                  >
                    {option}
                  </Text>
            ...
            <View className="flex-row justify-center mt-3 space-x-4">
              {["One-Time", "Daily", "Weekly", "Monthly", "Annually"].map(
                (option) => (
                  <TouchableOpacity
                    key={option}
